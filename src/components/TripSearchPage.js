...
export default class TripSearchPage extends React.Component {
  ...
  search(event) {
    ...
    fetch(
      `${BASE_URL}/routes?departure_city=${this.state.departureCity}&arrival_city=${this.state.arrivalCity}&order_by=${this.state.sortBy}`
    )
      .then((response) => response.json())
      .then((data) => {
  ...
  renderLoader() {
    return (
      <Container
        ...
        className="rounded shadow-lg mt-4 p-4 text-center"
      >
        <div className="text-muted">Search for routes...</div>
  ...
  renderTripList() {
    if (
      ...
      this.state.routeData.routeDetails
    ) {
      return (
        <Container
          style={{ background: "#fff" }}
          className="rounded shadow-lg mt-4 pb-4"
        >
          <Row>
            <Col>
  ...
  render() {
    return (
      <div className="trip-search-container">
        ...
        <Container
          fluid
          style={{ width: "100%", height: "100%", paddingTop: "24vh" }}
          className="travel-bg"
        >
          <div className="text-white pb-5 header-title">
            <h1>Find your best offer for travel</h1>
          </div>
                ...
                <Col md={4}>
                  <FormGroup>
                    ...
                    <Button
                      ...
                      active={this.state.sortBy === "Duration"}
                    >
                      Fastest Route
                    </Button>
                    <Button
                  ...
                  <Button
                    ...
                    disabled={
                      !(
                        ...
                        this.state.departureCity !== this.state.arrivalCity
                      )
                    }
                  >
                    Search
